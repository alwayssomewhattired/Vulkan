...
void ModelLoad::modelFileParse(aiScene* scene, aiMesh* mesh, size_t& vertexCount, 
...
void ModelLoad::loadModel(const std::string& path, ItemInterface& classReference) {
	...
	for (uint32_t i = 0; i < scene->mNumMeshes; i++) {
		...
		modelFileParse(mesh, vertexCount, vertices, indexType, indices, classReference, i, scene->mNumMeshes);
