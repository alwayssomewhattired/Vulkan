...
void GraphicsPipeline::createGraphicsPipeline(UniformBuffer& uniformBuffer) {
	...
	auto vertShaderCode = m_Shaders.readFile("shaders/vert.spv");
	auto fragShaderCode = m_Shaders.readFile("shaders/frag.spv");
	// print out values to see if they match the file sizes ^^^

	VkShaderModule vertShaderModule = m_Shaders.createShaderModule(vertShaderCode);
	VkShaderModule fragShaderModule = m_Shaders.createShaderModule(fragShaderCode);
	...
	pushConstantRanges.size = sizeof(glm::mat4) + sizeof(glm::vec4) + (sizeof(glm::uint) * 3);

	std::array<VkDescriptorSetLayout, 4> dsLayouts = {
		m_descriptorSetLayout.globalDescriptorSetLayout,
		m_descriptorSetLayout.meshMaterialDescriptorSetLayout,
...
void GraphicsPipeline::createMandelbulbComputePipeline() {
	auto compShaderCode = m_Shaders.readFile("shaders/mandelbulb.comp.spv");

	VkShaderModule compShaderModule = m_Shaders.createShaderModule(compShaderCode);
...
void GraphicsPipeline::createMandelbulbGraphicsPipeline() {

	auto vertShaderCode = m_Shaders.readFile("shaders/mandelbulb.vert.spv");
	auto fragShaderCode = m_Shaders.readFile("shaders/mandelbulb.frag.spv");
